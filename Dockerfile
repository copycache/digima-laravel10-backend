...
RUN apk add --no-cache dcron busybox-suid libcap curl zip unzip git supervisor

# Install PHP extensions
COPY --from=mlocati/php-extension-installer /usr/bin/install-php-extensions /usr/bin/
# RUN install-php-extensions intl bcmath gd pdo_mysql pdo_pgsql opcache redis uuid exif pcntl zip
RUN install-php-extensions intl bcmath gd pdo_mysql pdo_pgsql opcache exif pcntl zip
...
# Install composer
COPY --from=composer/composer:2.2.14 /usr/bin/composer /usr/local/bin/composer
