...
class MemberProductController extends MemberController
{
    ...
    public function get_level_item()
    {
        ...
        $cause_slot_ids = collect($items->items())->pluck('unilevel_points_cause_id')->unique();
        $item_ids = collect($items->items())->pluck('unilevel_item_id')->unique();

        $slots_map = Tbl_slot::whereIn('slot_id', $cause_slot_ids)->pluck('slot_no', 'slot_id');
        $item_map = Tbl_item::whereIn('item_id', $item_ids)->get()->keyBy('item_id');

        foreach ($items as $key => $value) {
            $items[$key]->buyer_name = $slots_map->get($value->unilevel_points_cause_id);
            $points = $item_map->get($value->unilevel_item_id);
            if ($points && $points->item_pv != 0) {
                $items[$key]->sum_points = $value->unilevel_points_amount;
            } else {
                $items[$key]->sum_points = 0;
            }
            $items[$key]->item_desc = $points;
        }
    ...
    public function search_product()
    {
        ...
        $return = Tbl_item::where('archived', 0)
            ->where(function ($query) {
                $query
                    ->where('item_availability', 'ecommerce')
    ...
    {
        $response['data'] = Tbl_orders_for_approval::where('user_id', Request::user()->id)
        ...
        if ($response['data'] != null) {
            ...
            $items_decoded = json_decode($response['data']['items']);
            $item_ids = collect($items_decoded)->pluck('item_id')->unique()->filter();
            $items_map = Tbl_item::whereIn('item_id', $item_ids)->get()->keyBy('item_id');

            foreach ($items_decoded as $key => $value) {
                $item = $items_map->get($value->item_id);
                if ($item) {
                    $response['item_details'][$key] = $item;
    ...
    public function simpleCheckout()
    {
        try {
            ...
            if (empty($receiver_contact))
                $receiver_contact = $user->contact_no ?? '';
