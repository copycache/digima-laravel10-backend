...
class MemberDashboardController extends MemberController
{
    ...
    {
        $keys = [
            'allow_slot_transfer', 'register_on_slot', 'register_your_slot',
            'product_activate', 'membership_kit_upgrade', 'lockdown_enable',
            'name_on_dropdown', 'earning_in_switch_slot', 'breakdown_gc',
            'breakdown_left_and_right', 'top_earners'
        ];
        $settings = Tbl_other_settings::whereIn('key', $keys)->get()->pluck('value', 'key');

        $response = [
            'allow_slot_transfer' => $settings->get('allow_slot_transfer'),
            'register_on_slot' => $settings->get('register_on_slot'),
            'register_your_slot' => $settings->get('register_your_slot', 0),
            'vortex_on' => Tbl_mlm_plan::where('mlm_plan_code', 'VORTEX_PLAN')->where('mlm_plan_enable', 1)->exists() ? 1 : 0,
            'product_activate' => $settings->get('product_activate', 1),
            'membership_kit_upgrade' => $settings->get('membership_kit_upgrade', 0),
            'lockdown_enable' => $settings->get('lockdown_enable', 0),
            'name_on_dropdown' => $settings->get('name_on_dropdown', 0),
            'earning_in_switch_slot' => $settings->get('earning_in_switch_slot', 0),
            'breakdown_gc' => $settings->get('breakdown_gc', 1),
            'breakdown_left_and_right' => $settings->get('breakdown_left_and_right', 1),
            'top_earners' => $settings->get('top_earners', 1),
        ];
    ...
    {
        $get_owner = Tbl_slot::where('slot_owner', Request::input('id'))->first();
        if ($get_owner) {
            $get_sponsor = Tbl_slot::where('slot_id', $get_owner->slot_sponsor)->value('slot_no');
            $get_owner->populated = $get_sponsor ?? '';
        }
    ...
    public static function dragonpay_history($filters = null, $export = 0)
    {
        ...
        $query = Tbl_dragonpay_transaction::where('tbl_dragonpay_transaction.buyer_slot_id', $filters['slot_id'])
            ->leftjoin('tbl_slot', 'tbl_slot.slot_id', 'tbl_dragonpay_transaction.buyer_slot_id')
            ->leftjoin('users', 'users.id', 'tbl_slot.slot_owner')
        ...
        foreach ($return as $value) {
            $value->items = json_decode($value->ordered_item);
            if ($value->items) {
                $all_item_ids = $all_item_ids->concat(collect($value->items)->pluck('item_id'));
            }
        }
        ...
        foreach ($return as $key => $value) {
            ...
            if ($value->items) {
                foreach ($value->items as $item) {
                    $item->item_sku = $items_map->get($item->item_id) ?? 'N/A';
                }
            }
        }
        return $return;
