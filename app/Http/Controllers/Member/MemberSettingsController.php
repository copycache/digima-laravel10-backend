...
class MemberSettingsController extends MemberController
{
	public function upload_profile(Request2 $request)
	{
		...
		if ($file->isValid()) {
			...
			User::where('id', Request::user()->id)->update($update);
			$response = 'success';
	...
	public function upload_id(Request2 $request)
	{
		...
		$path = 'mlm/valid_id';
		$storage_path = storage_path();
		$check_user = User::where('id', Request::user()->id)->first()->valid_id;
	...
	public function get_user_info()
	{
		...
		$slot_data = Tbl_slot::where('slot_id', $slot_id)
			->AchieversRankAttribute()
			->LivewellRankAttribute()
			->select('store_name', 'tbl_achievers_rank.achievers_rank_name', 'tbl_livewell_rank.livewell_rank_name')
			->first();

		$response->store_name = $slot_data->store_name ?? null;

		$points = Tbl_binary_points::where('binary_points_slot_id', $slot_id)
			->select(
				DB::raw('SUM(CASE WHEN binary_receive_left >= 0 THEN binary_receive_left ELSE 0 END) as left_points'),
				DB::raw('SUM(CASE WHEN binary_receive_right >= 0 THEN binary_receive_right ELSE 0 END) as right_points')
			)
			->first();

		$response->accumulated_left_points = $points->left_points ?? 0;
		$response->accumulated_right_points = $points->right_points ?? 0;

		$response->achievers_rank = $slot_data->achievers_rank_name ?? '---';
		$response->livewell_rank = $slot_data->livewell_rank_name ?? '---';
	...
	public function get_user_add_ons_info()
	{
		...
		if ($slot) {
			...
			if ($membership) {
				...
				$pluss = Tbl_unilevel_points::where('unilevel_points_slot_id', $slot->slot_id)
		...
		if ($response['user_info']) {
			...
			$response['user_info']['is_dynamic'] = Tbl_mlm_unilevel_settings::first()->is_dynamic ?? 0;
			...
			$binary_points = Tbl_binary_points::where('binary_points_slot_id', $slot_id)
				->select(DB::raw('SUM(binary_receive_left) as left_sum'), DB::raw('SUM(binary_receive_right) as right_sum'))
				->first();
			$response['user_info']['accumulated_left_points'] = $binary_points->left_sum ?? 0;
			$response['user_info']['accumulated_right_points'] = $binary_points->right_sum ?? 0;
		}
	...
	{
		$address_list = Tbl_address::where('tbl_address.user_id', Request::user()->id)
			->where('tbl_address.archived', 0)
			->leftjoin('tbl_receiver_infomation', 'tbl_receiver_infomation.address_id', 'tbl_address.address_id')
			->select('*', DB::raw('tbl_address.address_id as address_id'))
			->get();

		$regCodes = $address_list->pluck('regCode')->unique()->filter();
		$provCodes = $address_list->pluck('provCode')->unique()->filter();
		$citymunCodes = $address_list->pluck('citymunCode')->unique()->filter();
		$brgyCodes = $address_list->pluck('brgyCode')->unique()->filter();
		$islandGroups = $address_list->pluck('island_group')->unique()->filter();

		$regions = Refregion::whereIn('regCode', $regCodes)->pluck('regDesc', 'regCode');
		$provinces = Refprovince::whereIn('provCode', $provCodes)->pluck('provDesc', 'provCode');
		$cities = Refcitymun::whereIn('citymunCode', $citymunCodes)->pluck('citymunDesc', 'citymunCode');
		$brgys = Refbrgy::whereIn('brgyCode', $brgyCodes)->pluck('brgyDesc', 'brgyCode');
		$islands = Tbl_island_group::whereIn('id', $islandGroups)->pluck('island_group', 'id');

		foreach ($address_list as $key => $list) {
			$address_list[$key]['refregion'] = $regions->get($list->regCode);
			$address_list[$key]['refprovince'] = $provinces->get($list->provCode);
			$address_list[$key]['refcitymun'] = $cities->get($list->citymunCode);
			$address_list[$key]['refbrgy'] = $list->brgyCode;
			$address_list[$key]['brgyDesc'] = $brgys->get($list->brgyCode);

			$address_list[$key]['island_group_name'] = $islands->get($list->island_group);
