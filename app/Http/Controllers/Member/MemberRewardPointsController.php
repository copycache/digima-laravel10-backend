...
class MemberRewardPointsController extends MemberController
{
    ...
    public function list_of_claimed_redemption_items(Request $request)
    {
        ...
        $records = $data->orderByRaw("CASE WHEN tbl_claimed_reward_items.status = 'For Approval' THEN 0 ELSE 1 END")
    ...
    {   
        ...
        foreach($list_of_currency as $currency) {
            $available_wallet[$currency->currency_name] = Tbl_wallet::currency()
            ->where('slot_id', $request->slot_id)
            ->where("tbl_currency.currency_id", $currency->currency_id)
            ->first();
        }
