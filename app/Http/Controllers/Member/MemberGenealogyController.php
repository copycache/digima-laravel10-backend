...
class MemberGenealogyController extends MemberController
{
    ...
    public function unilevel()
    {
        ...
        $slot_ids = $data['_slot']->pluck('slot_id');
        $sponsor_ids = $data['_slot']->pluck('slot_sponsor')->unique()->filter();

        $recruits_map = Tbl_slot::whereIn('slot_sponsor', $slot_ids)->select('slot_sponsor', DB::raw('COUNT(*) as total'))->groupBy('slot_sponsor')->pluck('total', 'slot_sponsor');
        $sponsors_map = Tbl_slot::whereIn('slot_id', $sponsor_ids)->pluck('slot_no', 'slot_id');

        $first_date = Carbon::now()->startOfMonth()->format('Y-m-d');
        $end_date = Carbon::now()->endOfMonth()->format('Y-m-d');
        $ppv_map = Tbl_unilevel_points::whereIn('unilevel_points_slot_id', $slot_ids)
            ...
            ->select('unilevel_points_slot_id', DB::raw('SUM(unilevel_points_amount) as total'))
            ->groupBy('unilevel_points_slot_id')
            ->pluck('total', 'unilevel_points_slot_id');

        $unilevel_settings = Tbl_mlm_unilevel_settings::first();

        foreach ($data['_slot'] as $key => $value) {
            $data['_slot'][$key]->total_recruits = $recruits_map->get($value->slot_id) ?? 0;
            $data['_slot'][$key]->settings = $settings;
            $data['_slot'][$key]->is_dynamic = $unilevel_settings->is_dynamic ?? 'normal';

            $total_ppv = $ppv_map->get($value->slot_id) ?? 0;
            $data['_slot'][$key]->slot_personal_pv = number_format($total_ppv, 0);
            $data['_slot'][$key]->sponsor_username = $sponsors_map->get($value->slot_sponsor) ?? 'N/A';
        }
    ...
    public function get_placement_downline()
    {
        ...
        $sponsor_ids = collect($return->items())->pluck('slot_sponsor')->unique()->filter();
        $placement_ids = collect($return->items())->pluck('slot_placement')->unique()->filter();

        $user_names_map = Tbl_slot::join('users', 'users.id', '=', 'tbl_slot.slot_owner')
            ->whereIn('slot_id', $sponsor_ids->merge($placement_ids)->unique())
            ->pluck('name', 'slot_id');

        $placement_info_map = Tbl_slot::whereIn('slot_id', $placement_ids)->pluck('slot_no', 'slot_id');

        foreach ($return as $key => $value) {
            $return[$key]->sponsor_name = $user_names_map->get($value->slot_sponsor);
            $return[$key]->placement_name = $user_names_map->get($value->slot_placement);
            $return[$key]->placement_slotno = $placement_info_map->get($value->slot_placement);
        }
        return $return;
    }
    ...
    public function get_matrix_downline()
    {
        ...
        $sponsor_ids = collect($return->items())->pluck('slot_sponsor')->unique()->filter();
        $matrix_ids = collect($return->items())->pluck('matrix_sponsor')->unique()->filter();

        $user_names_map = Tbl_slot::join('users', 'users.id', '=', 'tbl_slot.slot_owner')
            ->whereIn('slot_id', $sponsor_ids->merge($matrix_ids)->unique())
            ->pluck('name', 'slot_id');

        $matrix_info_map = Tbl_slot::whereIn('slot_id', $matrix_ids)->pluck('slot_no', 'slot_id');

        foreach ($return as $key => $value) {
            $return[$key]->sponsor_name = $user_names_map->get($value->slot_sponsor);
            $return[$key]->matrix_name = $user_names_map->get($value->matrix_sponsor);
            $return[$key]->matrix_slotno = $matrix_info_map->get($value->matrix_sponsor);
        }
        return $return;
    }
    ...
    public function get_unilevel_downline()
    {
        ...
        $sponsor_ids = collect($return->items())->pluck('slot_sponsor')->unique()->filter();
        $sponsor_info_map = Tbl_slot::join('users', 'users.id', '=', 'tbl_slot.slot_owner')
            ->whereIn('slot_id', $sponsor_ids)
            ->select('slot_id', 'slot_no', 'name')
            ->get()
            ->keyBy('slot_id');

        foreach ($return as $key => $value) {
            $s_info = $sponsor_info_map->get($value->slot_sponsor);
            $return[$key]->sponsor_code = $s_info ? $s_info->slot_no : 'N/A';
            $return[$key]->sponsor_name = $s_info ? $s_info->name : 'N/A';
        }
        return $return;
    }
