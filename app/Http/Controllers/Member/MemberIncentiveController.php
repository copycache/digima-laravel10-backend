...
class MemberIncentiveController extends MemberController
{
    public function get_item(Request $request)
    {
        ...
        $setup_item_ids = $incentive_setups->pluck('item_id')->unique()->filter();
        $product_names_map = Tbl_item::whereIn('item_id', $setup_item_ids)->pluck('item_sku', 'item_id');

        // Process each filtered item
        $items->transform(function ($item) use ($incentive_setups, $purchase_counts, $product_names_map) {
            ...
            // Fetch the product name from map
            $item->product_name = $product_names_map->get($incentive_setup->item_id) ?? 'Unknown';
