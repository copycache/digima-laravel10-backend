...
class MemberExportController extends MemberController
{
    public function export_member_sponsor_list_csv()
    {
        ...
        if ($genealogy_type == 'BINARY') {
            ...
            $sponsor_ids = $response->pluck('slot_sponsor')->unique()->filter();
            $sponsors_map = Tbl_slot::whereIn('slot_id', $sponsor_ids)->pluck('slot_no', 'slot_id');

            foreach ($response as $key => $value) {
                $response[$key]->sponsor_code = $sponsors_map->get($value->slot_sponsor);
            }
    ...
    public function export_member_history_code_csv()
    {
        ...
        $slot_ids = $data
            ->pluck('from_slot')
            ->merge($data->pluck('to_slot'))
            ->merge($data->pluck('original_slot'))
            ->unique()
            ->filter();
        ...
        $counter = 0;
        $mapper = function ($list) use (&$counter, $slots_map) {
            $counter++;
            ...
            $from_slot_code = $slots_map->get($list->from_slot);
            $to_slot_code = $slots_map->get($list->to_slot);
            $original_slot_code = $slots_map->get($list->original_slot);
    ...
    public function export_member_history_code_pdf()
    {
        ...
        foreach ($data as $key => $list) {
            ...
            $data[$key]['from_slot_code'] = (object) ['slot_no' => $slots_map->get($list['from_slot'])];
            $data[$key]['to_slot_code'] = (object) ['slot_no' => $slots_map->get($list['to_slot'])];
            $data[$key]['original_slot_code'] = (object) ['slot_no' => $slots_map->get($list['original_slot'])];
        }
