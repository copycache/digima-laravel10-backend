...
class Code
{
	...
	public static function get($branch_id, $filter = null, $item_id = null, $paginate = null)
	{
		...
		if (isset($filter['search']) && $filter['search'] != 'null') {
			$data = $data->where(function ($query) use ($filter, $inventory) {
				$query
					->where('tbl_codes.code_inventory_id', $inventory->inventory_id)
					->where(function ($q) use ($filter) {
						$q
							->where('code_activation', 'like', '%' . $filter['search'] . '%')
	...
	public static function get_member_codes($user_id, $filter)
	{
		...
		foreach ($s as $key => $value) {
			...
			$return[$key]['user'] = User::where('id', $value->code_used_by)->select('name')->first();
	...
	public static function load_product_code($user_id, $slot_id, $key, $action, $paginate)
	{
	...
	public static function load_membership_code($user_id, $slot_id, $key, $action, $paginate)
	{
		...
		$membership = $action == 'filter' ? $key : Request::input('filter');
		...
		foreach ($return['code_list'] as $key => $value) {
			if ($value->code_used == 1) {
				$return['code_list'][$key]['name'] = User::where('id', $value->code_used_by)->value('name');
			}
		}
